...
public class ShiftingFMWave2 extends ShiftingFMWave {
    ...
    fun void play() {
        ...
        for (0 => int i; i < grains.numberOfGrains - 1; i++) {
            Std.mtof(getTransformation(grains.minHeartRate, grains.maxHeartRate, 24, 48, grains.heartRate[i])) => 
            float startCarFreq;
            
            Std.mtof(getTransformation(grains.minHeartRate, grains.maxHeartRate, 24, 48, grains.heartRate[i + 1])) => 
            float endCarFreq;
