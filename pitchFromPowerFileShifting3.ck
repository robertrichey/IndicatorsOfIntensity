...
SinOsc modulator => TriOsc carrier => NRev rev => Pan2 pan => dac;
// Envelope env =>
...
for (0 => int i; i < powerAverages.size() - 1; i++) {
    Std.mtof(getTransformation(minAverageSpeed, maxAverageSpeed, 72, 96, speedAverages[i])) => 
    float startCarFreq;
    
    Std.mtof(getTransformation(minAverageSpeed, maxAverageSpeed, 72, 96, speedAverages[i + 1])) => 
    float endCarFreq;
    
    
    getTransformation(minAveragePower, maxAveragePower, 0, 100, powerAverages[i]) => 
    float startModFreq;
    
    getTransformation(minAveragePower, maxAveragePower, 0, 100, powerAverages[i + 1]) => 
    float endModFreq;
    
    
    getTransformation(minAverageHeartRate, maxAverageHeartRate, 0.05, 0.6, heartRateAverages[i]) => 
    float startCarGain;
    
    getTransformation(minAverageHeartRate, maxAverageHeartRate, 0.05, 0.6, heartRateAverages[i + 1]) => 
    float endCarGain;
    
    
    getTransformation(minAverageCadence, maxAverageCadence, 1, 500, cadenceAverages[i]) => 
    float startModGain;
    
    getTransformation(minAverageCadence, maxAverageCadence, 1, 500, cadenceAverages[i + 1]) => 
    float endModGain;
