...
// crerate patch, keep quiet until OSC message is receved
Flute instrument1 => NRev rev1 => dac.left;
Flute instrument2 => NRev rev2 => dac.right;

0.1 => rev1.mix;
0.1 => rev2.mix;
0.8 => instrument1.gain;
0.8 => instrument2.gain;

400 => instrument1.freq;

800 => instrument2.freq;
...
fun void play1() {
    for (0 => int i; i < speedAverages.size(); i++) {
        ...
        minAveragePower, maxAveragePower, 0, rhythms.size()-1, powerAverages[i])) => int row;        
                
        for (0 => int i; i < rhythms[row].size(); i++) { 
            Std.mtof(Math.random2(60,71)) => instrument1.freq;
            math.random2f(0, 1) => float chance;
            
            if (chance > 0.15) {
                1 => instrument1.noteOn;
                rhythms[row][i]=> now;
...
fun void play2() {
    for (0 => int i; i < speedAverages.size(); i++) {
        Std.ftoi(getTransformation(
        minAverageSpeed, maxAverageSpeed, 0, rhythms.size()-1, speedAverages[i])) => int row;        
        
        for (0 => int i; i < rhythms[row].size(); i++) { 
            Std.mtof(Math.random2(60,71) + 12) => instrument2.freq;
            math.random2f(0.0, 1.0) => float chance;

            if (chance > 0.15) {
                1 => instrument2.noteOn;
                rhythms[row][i]=> now;
                1 => instrument2.noteOff;
            }
            else {
                rhythms[row][i]=> now;
            }
