...
spork ~ play(instrument2, minAverageSpeed, maxAverageSpeed, speedAverages, 2);
spork ~ play(instrument3, minAverageCadence, maxAverageCadence, cadenceAverages, 1);
spork ~ play(instrument4, minAverageHeartRate, maxAverageHeartRate, heartRateAverages, 0);
...
fun void play(StkInstrument instrument, float oldBottom, float oldTop, float values[], int octave) {
    for (0 => int i; i < speedAverages.size(); i++) {
        ...
        for (0 => int i; i < rhythms[row].size(); i++) { 
            Std.mtof(Math.random2(36, 49) + 12 * octave) => instrument.freq;
